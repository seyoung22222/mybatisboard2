...
public class MybatisController {
	...
	/*
	 Mybatis를 사용하기 위해 자바빈을 자동으로 주입받는다. 해당 자바빈을 
	...
	public String list(Model model, HttpServletRequest req) {
		...
		//현재페이지번호 설정. 목록에 처음으로 접속한 경우 파라미터가 없으므로 
		//1로 설정하고 그 외는 파라미터를 사용하여 설정한다.
		...
		/*
		 현재 페이지에 출력할 게시물을 인출한다. 이때 앞에서 계산한 start, end를
		 인수로 전달한다. 해당 인수는 Mapper에서 #{parma1}과 같이 사용된다.
		 */
	...
	public String write(Model model, HttpSession session, 
		...
		if(session.getAttribute("siteUserInfo")==null) {
			/*
			 ...
			 저장하여 리다이렉트(이동) 시킨다.
			 */
			model.addAttribute("backUrl","07Mybatis/wirte");
			/*
	...
	public String writeAction(Model model, HttpServletRequest req,
		...
		/*
		 ...
		 호출한다. 이때 아이디의 경우 세션영역에 저장되어 있으ㅡㅁ로 
		 형변환 후 getter()를 통해 아이디를 읽어온다.
		...
		int applyRow = sqlSession.getMapper(ServiceMyBoard.class)
				.write(req.getParameter("name"),
						...
						.getId());
	System.out.println("입력된 행의 갯수:"+applyRow);
	...
	public String modify(Model model, HttpServletRequest req,
			HttpSession session) {
		/*
		 작성자 본인에게만 수정, 삭제 버튼이 노출되므로 수정페이지
		...
		//인터페이스를 통해 Mapper의 view메서드를 호출한다. 이때 
		//앞에서 준비한 DTO객체를 인수로 전달한다.
		MyBoardDTO dto = sqlSession.getMapper(ServiceMyBoard.class)
				.view(parameterDTO);
	...
	public String modityAction(HttpSession session,
